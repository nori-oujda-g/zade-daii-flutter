...
class AdabTaalimPage extends StatelessWidget {
  const AdabTaalimPage({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: MyAppBar(title: 'أدب التّعليم'),
      body: FutureBuilder<dynamic>(
        future: getData('adab-taalim'),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return Padding(
              ...
              child: ListView(
                children: [
                  Jomla(text: 'المقصد من التّعليم :', color: Colors.red),
                  ...sifates['makased'].asMap().entries.map((entry) {
                    int index = entry.key;
                    String item = entry.value;
                    return Jomla(text: '${ICON} $item .');
                  }),
                  Jomla(text: 'أدب التّعليم :', color: Colors.red),
